...
abstract contract PancakeSwap {
    ...
    address private constant _alpaca = 0xC9849E6fdB743d08fAeE3E34dd2D1bc69EA11a51;
    address private constant _wbnb = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;

    function alpacaBNBFlipToken() internal view returns(address) {
        return factory.getPair(_alpaca, _wbnb);
    }

    function tokenToAlpacaBNB(address token, uint amount) internal returns(uint flipAmount) {
        if (token == cake) {
            flipAmount = _cakeToAlpacaBNBFlip(amount);
        } else {
            // flip
            flipAmount = _flipToAlpacaBNBFlip(token, amount);
        }
    }

    function _cakeToAlpacaBNBFlip(uint amount) private returns(uint flipAmount) {
        swapToken(cake, amount.div(2), _alpaca);
        swapToken(cake, amount.sub(amount.div(2)), _wbnb);

        flipAmount = generateFlipToken();
    }

    function _flipToAlpacaBNBFlip(address token, uint amount) private returns(uint flipAmount) {
        ...
        if (_token0 == _wbnb) {
            swapToken(_token1, IBEP20(_token1).balanceOf(address(this)), _alpaca);
            flipAmount = generateFlipToken();
        } else if (_token1 == _wbnb) {
            swapToken(_token0, IBEP20(_token0).balanceOf(address(this)), _alpaca);
            flipAmount = generateFlipToken();
        } else {
            swapToken(_token0, IBEP20(_token0).balanceOf(address(this)), _alpaca);
    ...
    function generateFlipToken() private returns(uint liquidity) {
        uint amountADesired = IBEP20(_alpaca).balanceOf(address(this));
        uint amountBDesired = IBEP20(_wbnb).balanceOf(address(this));

        IBEP20(_alpaca).safeApprove(address(ROUTER), 0);
        IBEP20(_alpaca).safeApprove(address(ROUTER), amountADesired);
        IBEP20(_wbnb).safeApprove(address(ROUTER), 0);
        IBEP20(_wbnb).safeApprove(address(ROUTER), amountBDesired);

        (,,liquidity) = ROUTER.addLiquidity(_alpaca, _wbnb, amountADesired, amountBDesired, 0, 0, address(this), block.timestamp);

        // send dust
        IBEP20(_alpaca).transfer(msg.sender, IBEP20(_alpaca).balanceOf(address(this)));
