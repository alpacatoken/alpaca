...
contract CakeFlipVaultTester is IStrategyLegacy, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    constructor(uint _pid) public {
        ...
        setMinter(IAlpacaMinter(0x0B4A714AAf59E46cb1900E3C031017Fd72667EfE));
    }
    ...
    // return cakeAmount, alpacaAmount, 0
    function profitOf(address account) override public view returns (uint _usd, uint _alpaca, uint _bnb) {
        ...
        if (address(minter) != address(0) && minter.isMinter(address(this))) {
            ...
            // alpaca amount
            _alpaca = minter.amountAlpacaToMint(bnbValue);
        } else {
            _usd = amount;
            _alpaca = 0;
        }
    ...
    function setMinter(IAlpacaMinter _minter) public onlyOwner {
        // can zero
        minter = _minter;
