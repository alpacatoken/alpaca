...
contract AlpacaPool is IStrategyLegacy, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    /* ========== STATE VARIABLES ========== */

    IBEP20 public rewardsToken; // alpaca/bnb flip
    IBEP20 public constant stakingToken = IBEP20(0xC9849E6fdB743d08fAeE3E34dd2D1bc69EA11a51);   // alpaca
    ...
    function withdrawableBalanceOf(address account) override public view returns (uint) {
        if (block.timestamp > timestamp90DaysAfterPresaleEnds) {
            // unlock all presale alpaca after 90 days of presale
            return _balances[account];
            ...
            uint alpacaSupply = stakingToken.totalSupply().sub(stakingToken.balanceOf(deadAddress));
            if (soldInPresale >= alpacaSupply) {
                return _balances[account].sub(_presaleBalance[account]);
            }
            uint alpacaNewMint = alpacaSupply.sub(soldInPresale);
            if (alpacaNewMint >= soldInPresale) {
                return _balances[account];
            }

            uint lockedRatio = (soldInPresale.sub(alpacaNewMint)).mul(1e18).div(soldInPresale);
            uint lockedBalance = _presaleBalance[account].mul(lockedRatio).div(1e18);
            return _balances[account].sub(lockedBalance);
        }
    }

    function profitOf(address account) override public view returns (uint _usd, uint _alpaca, uint _bnb) {
        _usd = 0;
        _alpaca = 0;
    ...
    function apy() override public view returns(uint _usd, uint _alpaca, uint _bnb) {
        ...
        uint alpacaPrice = helper.tokenPriceInBNB(address(stakingToken));
        uint flipPrice = helper.tvlInBNB(address(rewardsToken), 1e18);

        _usd = 0;
        _alpaca = 0;
        _bnb = rewardPerTokenPerSecond.mul(365 days).mul(flipPrice).div(alpacaPrice);
    }
    ...
    function _flipToWBNB(uint amount) private returns(uint reward) {
        ...
        (uint rewardAlpaca,) = ROUTER.removeLiquidity(
            address(stakingToken), wbnb,
            amount, 0, 0, address(this), block.timestamp);
        address[] memory path = new address[](2);
        path[0] = address(stakingToken);
        path[1] = wbnb;
        ROUTER.swapExactTokensForTokens(rewardAlpaca, 0, path, address(this), block.timestamp);
    ...
    function info(address account) override external view returns(UserInfo memory) {
        ...
        (uint usd, uint alpaca, uint bnb) = profitOf(account);
        profit.usd = usd;
        profit.alpaca = alpaca;
        profit.bnb = bnb;
